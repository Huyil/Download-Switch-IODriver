 ...
 *        @arg CW_GPIOA CW_GPIOB 
 * @param GPIO_Init 参见GPIO_InitTypeDef的定义
 */
void GPIO_Init(GPIO_TypeDef  *GPIOx, GPIO_InitTypeDef *GPIO_InitCFG)
{
  if(GPIO_InitCFG->Pins == 0)
    return;

//    switch ((uint32_t)GPIOx)
//    {
//        case (uint32_t)CW_GPIOA:
//            CW_SYSCTRL->AHBEN |= SYSCTRL_KEY | SYSCTRL_AHBEN_GPIOA_Msk;
//            break;
//        case (uint32_t)CW_GPIOB:
//            CW_SYSCTRL->AHBEN |= SYSCTRL_KEY | SYSCTRL_AHBEN_GPIOB_Msk;
//            break;
//        case (uint32_t)CW_GPIOC:
//            CW_SYSCTRL->AHBEN |= SYSCTRL_KEY | SYSCTRL_AHBEN_GPIOC_Msk;
//            break;
//        default:return;
//    }

    if (GPIO_InitCFG->Mode == GPIO_MODE_ANALOG)      //模拟功能
    {
        REGBITS_SET(GPIOx->ANALOG, GPIO_InitCFG->Pins);
    }
    else
    {
        REGBITS_CLR(GPIOx->ANALOG, GPIO_InitCFG->Pins);                  //数字功能

        if ((GPIO_InitCFG->Mode & GPIO_MODE_INPUT) == GPIO_MODE_INPUT)  //输入模式
        {
            REGBITS_SET(GPIOx->DIR, GPIO_InitCFG->Pins);

            if ((GPIO_InitCFG->IT & GPIO_IT_RISING) == GPIO_IT_RISING)
            {
                REGBITS_SET(GPIOx->RISEIE, GPIO_InitCFG->Pins);
            }else REGBITS_CLR(GPIOx->RISEIE, GPIO_InitCFG->Pins);

            if ((GPIO_InitCFG->IT & GPIO_IT_FALLING) == GPIO_IT_FALLING)
            {
                REGBITS_SET(GPIOx->FALLIE, GPIO_InitCFG->Pins);
            }else REGBITS_CLR(GPIOx->FALLIE, GPIO_InitCFG->Pins);
        }
        else                                                           //输出模式
        {
            REGBITS_CLR(GPIOx->DIR, GPIO_InitCFG->Pins);

            if (GPIO_InitCFG->Mode == GPIO_MODE_OUTPUT_PP)
            {
                REGBITS_CLR(GPIOx->OPENDRAIN, GPIO_InitCFG->Pins);
            }
            else
            {
                REGBITS_SET(GPIOx->OPENDRAIN, GPIO_InitCFG->Pins);
            }
        }

        if (GPIO_InitCFG->Pull == GPIO_PULL_UP)
        {
            REGBITS_SET(GPIOx->PUR, GPIO_InitCFG->Pins);               
        }else{
            REGBITS_CLR(GPIOx->PUR, GPIO_InitCFG->Pins);
        }            
    }
}
...
/**
 * @brief:
 *        L011:  将PA13/PA14设置成GPIO
 *
 */
void GPIO_SWD2GPIO(void)
{
    CW_SYSCTRL->CR2 =  CW_SYSCTRL->CR2 | (0x5A5A0000 | SYSCTRL_CR2_SWDIO_Msk);
}

//=============================================================================
//
/**
 * @brief 将PA13/SWDIO PA14/SWCLK

 *
 */
void GPIO_GPIO2SWD(void)
{
    CW_SYSCTRL->CR2 =  0x5A5A0000 | (CW_SYSCTRL->CR2 & (~(SYSCTRL_CR2_SWDIO_Msk)));
}

/**
 * @brief 将PB07设置成GPIO，只能输入
 *
 */
...
//=============================================================================
//配置端口辅助功能
//请使用CW32L011_gpio.h中的宏定义来配置每个端口的辅助功能
