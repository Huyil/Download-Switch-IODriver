...
{   
    ...
    ADC_InitStruct->ADC_BgrEn ? (CW_ADC->CR_f.BGREN = 1) : (CW_ADC->CR_f.BGREN = 0);
    ADC_InitStruct->ADC_TempSensorEn ? (CW_ADC->CR_f.TSEN = 1) : (CW_ADC->CR_f.TSEN = 0);
...
void ADC_BGREnable(boolean_t _bool)
{
    CW_ADC->CR_f.BGREN = _bool ? 1 : 0;
}
void ADC_TSEnable(boolean_t _bool)
{
    CW_ADC->CR_f.TSEN = _bool ? 1 : 0;
}
