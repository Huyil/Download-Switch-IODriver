...
#ifndef __CW32L011_RTC_H
#define __CW32L011_RTC_H
...
typedef struct
{
    uint32_t reloadValue; // 唤醒定时器重载值
    uint32_t clockSource; // 时钟源选择 (0: RTCCLKD, 1: TICKCLK)
    uint32_t prescaler;   // 预分频值 (0: DIV2, 1: DIV4, 2: DIV8, 3: DIV16)
    uint8_t enableInterrupt; // 是否使能中断 (0: 禁用, 1: 使能)
} RTC_AWTTypeDef;
...
#define RTC_RTCCLK_FROM_HSE                     (1UL << 8)
#define RTC_RTCCLK_FROM_LSI                     (2UL << 8)
#define RTC_RTCCLK_FROM_HSIOSC                  (3UL << 8)


#define IS_RTC_RTCCLK_SOURCE(src)               (((src) == RTC_RTCCLK_FROM_LSE) || \
                                                ((src) == RTC_RTCCLK_FROM_HSE) || \
                                                ((src) == RTC_RTCCLK_FROM_LSI) || \
                                                ((src) == RTC_RTCCLK_FROM_HSIOSC))
...
#define RTC_AWTSOURCE_FROM_RTCCLKD             (0x0)
#define RTC_AWTSOURCE_FROM_TICKCLK             (0x1)

#define IS_RTC_AWTTIMER_SOURCE(clk)             ((clk) == RTC_AWTSOURCE_FROM_RTCCLKD ||\
                                                 (clk) == RTC_AWTSOURCE_FROM_TICKCLK)

// AWT预分频器取值宏定义
#define RTC_AWTPRS_DIV2  0x00  // 预分频值为2
#define RTC_AWTPRS_DIV4  0x01  // 预分频值为4
#define RTC_AWTPRS_DIV8  0x02  // 预分频值为8
#define RTC_AWTPRS_DIV16 0x03  // 预分频值为16
...
#define RTC_RTCCLK_HEX128()                     (CW_RTC->CR1_f.SOURCE = RTC_RTCCLK_FROM_HEX_128)
#define RTC_RTCCLK_HEX256()                     (CW_RTC->CR1_f.SOURCE = RTC_RTCCLK_FROM_HEX_256)
#define RTC_RTCCLK_HEX512()                     (CW_RTC->CR1_f.SOURCE = RTC_RTCCLK_FROM_HEX_512)
#define RTC_RTCCLK_HEX1024()                    (CW_RTC->CR1_f.SOURCE = RTC_RTCCLK_FROM_HEX_1024)
...
#define RTC_AWT_PSC_DIV2                        (0UL)
#define RTC_AWT_PSC_DIV4                        (1UL)
#define RTC_AWT_PSC_DIV8                        (2UL)
#define RTC_AWT_PSC_DIV16                       (3UL)
#define IS_RTC_AWT_PSC(PSC)                     ((PSC) < 4UL)
...
//============================================================
//  cw32l011 RTC HAL API Function（Total： 14），及所有位域的宏命令
//============================================================
...
void RTC_SetPrescale(uint8_t PSC1,uint32_t PSC2);
...
void RTC_SetAWTPrescale(uint8_t RtcAwtPscValue);
uint8_t RTC_BinToBCD(uint8_t Value);
uint8_t RTC_BCDToBin(uint8_t Value);
void RTC_SetRTCCLKD_Prescaler(uint32_t prescalerValue);
void RTC_SetTICKCLK_Prescaler(uint32_t prescalerValue);
